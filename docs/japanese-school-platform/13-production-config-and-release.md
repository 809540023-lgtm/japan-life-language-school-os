...
- `/school-platform`
  AI 日語補習班平台主產品
- `/marketplace`
  已導向主站 `/school-platform` 的 legacy 入口（保留提示）
- `/legacy/marketplace`
  仍保留舊的二手商品頁（如需臨時使用）
...
- `SCHOOL_PLATFORM_ZOOM_ACCOUNT_ID`
- `SCHOOL_PLATFORM_ZOOM_CLIENT_ID`
- `SCHOOL_PLATFORM_ZOOM_CLIENT_SECRET`
- `SCHOOL_PLATFORM_ZOOM_USER_ID`
- `SCHOOL_PLATFORM_ZOOM_API_BASE_URL`
- `SCHOOL_PLATFORM_ZOOM_OAUTH_BASE_URL`
- `SCHOOL_PLATFORM_ZOOM_TIMEZONE`
...
9. 做一筆真實小額付款驗證
10. 驗證通知外發與 retry
11. 驗證 Zoom 班級連結或 Zoom API 自動建會議
...
5. LINE token 是否遺漏
6. Zoom API 憑證是否遺漏，或 Zoom 班級是否仍缺 meeting_url
