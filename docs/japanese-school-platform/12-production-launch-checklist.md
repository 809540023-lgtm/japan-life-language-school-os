...
- 確認 `/marketplace` 已改為導向主站 `/school-platform`
- 確認保留的舊 marketplace 檢視在 `/legacy/marketplace`
...
- `GET /school-platform/admin/messages`
  ...
  - 失敗通知可單筆 retry
  - queued 通知可批次 drain
- 驗證至少各一筆（建議加 `--require-sent`）：
  - Email 寄送成功
  - LINE 推播成功或明確標示為 optional
  - in-app 通知正常顯示
- 驗證指令：
  - `python3 scripts/verify_school_platform_notification_delivery.py --base-url https://your-school-platform-domain --channels email,line --require-sent`
  - `python3 scripts/test_school_platform_notification_providers.py --channel email --require-sent`
  - `python3 scripts/test_school_platform_notification_providers.py --channel line --recipient U... --require-sent`
