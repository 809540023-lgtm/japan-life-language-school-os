...
class SchoolPlatformApiTest(unittest.TestCase):
    ...
    def test_public_courses_and_dashboard(self) -> None:
        ...
        self.assertIn("Japan Life Language School OS", marketplace_response.text)
        self.assertIn("/school-platform", marketplace_response.text)

        home_page_response = self.client.get("/school-platform")
        self.assertEqual(home_page_response.status_code, 200)
        self.assertIn("一個入口", home_page_response.text)
        self.assertIn("學員學習中心", home_page_response.text)
        self.assertIn("教師教學中心", home_page_response.text)
        self.assertIn("加盟合作中心", home_page_response.text)
        self.assertIn("學校營運後台", home_page_response.text)
        self.assertIn("整個平台只走一條清楚的營運路徑", home_page_response.text)
        ...
        self.assertIn("AI 教學規劃", home_page_response.text)
        ...
        franchise_page_response = self.client.get("/school-platform/franchise-vap")
        self.assertEqual(franchise_page_response.status_code, 200)
        self.assertIn("招商入口", franchise_page_response.text)
        self.assertIn("學員教學", franchise_page_response.text)
        self.assertIn("營運後台", franchise_page_response.text)
        self.assertIn('class="signal-pill"', franchise_page_response.text)
        self.assertNotIn("node.addEventListener('click',", franchise_page_response.text)
        self.assertIn('href="/school-platform/admin"', franchise_page_response.text)
        self.assertIn('href="/school-platform/franchise-vap"', franchise_page_response.text)
        self.assertIn('href="/school-platform/student-portal?email=web-enrollment%40example.com"', franchise_page_response.text)
        ...
        storage_backend = system_storage_response.json()["data"]["backend"]
        self.assertIn(storage_backend, {"json", "postgres"})
        ...
        if storage_backend == "json":
            ...
            self.assertEqual(system_storage_response.json()["data"]["repository_mode"], "domain_tables")
            self.assertIn("capabilities", system_storage_response.json()["data"])
            self.assertTrue(system_storage_response.json()["data"]["capabilities"]["query_supported"])
            self.assertTrue(system_storage_response.json()["data"]["capabilities"]["partial_write_supported"])
            self.assertTrue(system_storage_response.json()["data"]["capabilities"]["row_level_write_supported"])
            self.assertGreaterEqual(system_storage_response.json()["data"]["mutation_table_count"], 1)
        ...
        self.assertIn("zoom_provider", system_storage_response.json()["data"])
        self.assertIn("live_class_delivery", system_storage_response.json()["data"])
        ...
        if storage_backend == "json":
            self.assertIn("skipped", system_cutover_response.json()["data"])
            ...
            self.assertTrue(system_cutover_response.json()["data"]["success"])
            self.assertEqual(system_cutover_response.json()["data"]["backend"], "postgres")
            self.assertIn("record_counts", system_cutover_response.json()["data"])
        ...
        self.assertIn("Zoom Live Classes", system_page_response.text)
    ...
    def test_student_schedule_and_notifications_center_pages(self) -> None:
        ...
        self.assertIn("進入 Zoom 教室", schedule_page_response.text)
    ...
    def test_class_api_accepts_zoom_link_and_rejects_non_zoom_meeting_url(self) -> None:
        headers = self.login_headers()
        course_slug = self.client.get("/school-platform/api/public/courses").json()["data"][0]["slug"]
        ...
        valid_response = self.client.post(
            ...
            json={
                ...
                "meeting_url": "https://us06web.zoom.us/j/12345678901?pwd=abc123",
                "status": "open",
            },
            headers=headers,
        )
        self.assertEqual(valid_response.status_code, 200)
        self.assertEqual(valid_response.json()["data"]["meeting_url"], "https://us06web.zoom.us/j/12345678901?pwd=abc123")
        ...
        invalid_response = self.client.post(
            ...
            json={
                ...
                "capacity": 8,
                "location_label": "Zoom Live",
                "meeting_url": "https://meet.google.com/not-zoom",
                "status": "open",
            },
            headers=headers,
        )
        self.assertEqual(invalid_response.status_code, 400)
        self.assertIn("zoom.us", invalid_response.text)
    ...
    def test_messages_smoke_test_api(self) -> None:
        response = self.client.post(
            "/school-platform/api/messages/smoke-test",
            headers=self.login_headers(),
            json={
                "channel": "email",
                "recipient": "smoke@example.com",
                ...
                "content": "Testing provider smoke endpoint via API.",
            },
        )
        self.assertEqual(response.status_code, 200)
        payload = response.json()["data"]
        self.assertIn("notification", payload)
        self.assertEqual(payload["notification"]["type"], "provider_smoke_test")
        self.assertEqual(payload["notification"]["channel"], "email")
        self.assertIn(payload["notification"]["status"], {"queued", "sent", "suppressed", "failed"})
    ...
    def test_admin_ai_teaching_page(self) -> None:
        class_id = self.client.get("/school-platform/api/public/classes/open").json()["data"][0]["id"]
        response = self.client.get(
            f"/school-platform/admin/ai-teaching?class_id={class_id}&lesson_focus=租屋會話&duration_minutes=80&source_text=請幫我預約明天的試聽課"
        )
        self.assertEqual(response.status_code, 200)
        self.assertIn("AI 教案草稿中心", response.text)
        self.assertIn("教學目標", response.text)
        self.assertIn("租屋會話", response.text)
        self.assertIn("雙向即時翻譯教學卡", response.text)
        self.assertIn("翻譯結果摘要", response.text)
    ...
    def test_ai_bidirectional_translation_api(self) -> None:
        headers = self.login_headers()
        class_id = self.client.get("/school-platform/api/public/classes/open").json()["data"][0]["id"]
        response = self.client.post(
            "/school-platform/api/ai/bidirectional-translation",
            headers=headers,
            json={
                "class_id": class_id,
                "source_text": "請幫我預約明天的試聽課。",
                "lesson_focus": "試聽預約",
            },
        )
        self.assertEqual(response.status_code, 200)
        payload = response.json()["data"]
        self.assertEqual(payload["source_text"], "請幫我預約明天的試聽課。")
        self.assertIn("forward_translation", payload)
        self.assertIn("reverse_translation", payload)
        self.assertIn("teaching_notes", payload)
        self.assertIn("classroom_prompts", payload)
    ...
    def test_student_dashboard_and_notifications(self) -> None:
        ...
        classes_response = self.client.get("/school-platform/api/public/classes/open").json()["data"]
        if classes_response:
            class_item = classes_response[0]
            self.client.post(
                "/school-platform/api/teaching-materials",
                headers=headers,
                json={
                    ...
                    "class_id": class_item["id"],
                    "title": "測試教材",
                    "description": "學員端測試用教材",
                    "material_url": "https://example.com/materials/test.pdf",
                    "owner_type": "platform",
                    "visibility": "enrolled_only",
                    "status": "published",
                    "created_by": "自動化測試",
    ...
    def test_course_detail_page(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn("學員選定課程後，右側會看到這些內容", response.text)
        self.assertIn("報名後入口預覽", response.text)
        self.assertIn("未來實際顯示情境", response.text)
