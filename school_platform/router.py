...
from school_platform.zoom_runtime import is_zoom_delivery, is_zoom_meeting_url
...
def _course_scenario_key(course_type: str) -> str:
    course_type = (course_type or "").strip()
    if "jlpt" in course_type.lower():
        return "jlpt"
    if "職場" in course_type:
        return "work"
    if "生活" in course_type:
        return "life"
    return "other"


def _course_scenario_label(course_type: str) -> str:
    key = _course_scenario_key(course_type)
    return {"life": "生活日語", "work": "職場日語", "jlpt": "JLPT", "other": "其他場景"}.get(key, "其他場景")


def _course_level_key(level: str) -> str:
    return (level or "").strip().lower() or "未設定"


def _load_site_entry_rows(
    *,
    group_filter: str | None = None,
    method_filter: str | None = None,
) -> list[dict[str, str]]:
    catalog_path = Path(__file__).resolve().parent.parent / "docs" / "url-index" / "all_app_urls.csv"
    ...
    with catalog_path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for raw in reader:
            entry = {key: str(value or "").strip() for key, value in raw.items() if key is not None}
            if not entry:
                continue
            entry_group = entry.get("group", "").strip()
            entry_method = entry.get("method", "").strip().upper()
            if group_filter and group_filter.strip() and entry_group != group_filter.strip():
                continue
            if method_filter and method_filter.strip() and entry_method != method_filter.strip().upper():
                continue
            ...
            rows.append(
                {
                    "group": entry_group or "unknown",
                    "method": entry_method or "GET",
                    "path": entry["path"],
                    "url": entry["url"],
                }
            )
    rows.sort(key=lambda item: (item["group"].lower(), item["method"], item["path"]))
    return rows
...
def _material_source_html(material: TeachingMaterialRecord) -> str:
    if not material.material_url or material.storage_kind == "external_url":
        return ""
    safe_material_url = _normalize_material_download_url(material.material_url)
    if not safe_material_url:
        return "<p>來源：授權連結未核可</p>"
    return f"<p>來源：<a href='{escape(safe_material_url)}'>{escape(material.material_url)}</a></p>"


def _safe_local_redirect(raw: str | None, *, allow_external: bool = False) -> str:
    if not raw:
        return ""
    parsed = urlparse(raw)
    if parsed.scheme or parsed.netloc:
        if not allow_external:
            return ""
        allowed_hosts = _normalized_material_url_allowlist_hosts()
        if not parsed.hostname:
            return ""
        if parsed.hostname.lower() not in allowed_hosts:
            return ""
        if parsed.scheme == "http" and parsed.hostname not in {"127.0.0.1", "localhost", "school-platform.local"} and not store._is_local_demo_mode():
            return ""
        if parsed.scheme != "https" and not (store._is_local_demo_mode() and parsed.hostname in {"127.0.0.1", "localhost", "school-platform.local"}):
            return ""
        return raw
    path = parsed.path or ""
    if not path.startswith("/school-platform"):
        return ""
    if path.startswith("/school-platform//"):
        return ""
...
def _normalize_material_download_url(raw_url: str | None) -> str:
    return _safe_local_redirect(raw_url, allow_external=True)


def _normalized_material_url_allowlist_hosts() -> set[str]:
    allowlist = {
        item.strip().lower()
        for item in (store.settings.material_url_allowlist_hosts or "").split(",")
        if item.strip()
    }
    if store._is_local_demo_mode():
        allowlist.update({"127.0.0.1", "localhost", "school-platform.local"})
    return allowlist


def _meeting_entry_button(
    meeting_url: str | None,
    *,
    css_class: str = "btn alt",
    label: str | None = None,
) -> str:
    if not meeting_url:
        return ""
    button_label = label or ("進入 Zoom 教室" if is_zoom_meeting_url(meeting_url) else "進入線上教室")
    return (
        f"<a class='{escape(css_class)}' href='{escape(meeting_url)}' "
        "target='_blank' rel='noreferrer noopener'>"
        f"{escape(button_label)}</a>"
    )


def _meeting_status_chip(location_label: str, meeting_url: str | None) -> str:
    if meeting_url:
        return "<span class='chip'>Zoom 已設定</span>" if is_zoom_meeting_url(meeting_url) else "<span class='chip'>線上連結已設定</span>"
    if is_zoom_delivery(location_label, meeting_url):
        return "<span class='chip'>Zoom 待設定</span>"
    return ""
...
def _create_material_from_form(
    *,
    ...
    if normalized_url is not None:
        normalized_url = _normalize_material_download_url(normalized_url)
        if not normalized_url:
            raise HTTPException(status_code=400, detail="Material URL is blocked")
...
def _safe_platform_redirect(raw: str | None) -> str:
    if not raw:
        return "/school-platform/admin"
    parsed = urlparse(raw)
    if not parsed.path or not parsed.path.startswith("/school-platform"):
        return "/school-platform/admin"
    safe = parsed.path
    if parsed.query:
...
def _build_login_cookie_response(redirect_to: str, token: str, request: Request | None = None) -> Response:
    target = _safe_platform_redirect(redirect_to)
    response = RedirectResponse(url=target, status_code=303)
    response.set_cookie(
        key="school_platform_access_token",
        value=token,
        httponly=True,
        path="/",
        samesite="lax",
...
def _login_error_message(message: str | None) -> str:
    if not message:
        return ""
    safe_message = escape(message)
    return f"<article class='card'><p class='error'>登入失敗：{safe_message}</p></article>"
        ...
        <style>
          ...
          * {{ box-sizing: border-box; }}
          ...
          .entry-quick-card {{
            ...
            padding: 18px;
            background:
              linear-gradient(180deg, rgba(255,255,255,.98), rgba(247,241,232,.92)),
              radial-gradient(circle at top right, rgba(203,156,68,.12), transparent 42%);
            box-shadow: var(--shadow-sm);
          }}
          .entry-quick-card:hover {{
          ...
          .filter-chip {{
            ...
            padding: 8px 12px;
            border-radius: 999px;
            border: 1px solid rgba(39, 58, 54, .14);
          ...
          .filter-chip.active {{
            background: linear-gradient(135deg, rgba(195,93,52,.14), rgba(199,154,72,.18));
            border-color: rgba(195,93,52,.25);
            color: var(--ink);
          }}
          .entry-section-head {{
            margin: 0 0 12px;
            border-bottom: 1px dashed rgba(39, 58, 54, .16);
            padding-bottom: 10px;
            color: var(--forest-strong);
            font-size: 13px;
            font-weight: 900;
          ...
          .entry-table-wrap {{
            border: 1px solid rgba(39, 58, 54, .1);
            border-radius: 20px;
            overflow: hidden;
          ...
          .entry-table td {{
            padding: 12px 10px;
            border-bottom: 1px solid rgba(39, 58, 54, .08);
            text-align: left;
            vertical-align: top;
          }}
          .entry-table th {{
            background: rgba(35,75,68,.06);
            color: var(--forest);
            font-size: 12px;
            letter-spacing: .08em;
            text-transform: uppercase;
          ...
          .entry-table td {{ color: var(--ink); word-break: break-word; }}
          .entry-table a {{ color: var(--accent-strong); }}
          .entry-table .mono {{ font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; }}
          .entry-table .chip {{
            background: linear-gradient(135deg, rgba(35,75,68,.08), rgba(199,154,72,.12));
            color: var(--forest-strong);
            margin-bottom: 3px;
          }}
          ...
          .error {{
            color: var(--danger);
            border-color: rgba(177, 58, 50, .28);
            background: rgba(177, 58, 50, .05);
          }}
          ...
          .course-filter-row {{
            ...
            padding: 16px 18px;
            border-radius: 22px;
            border: 1px solid rgba(31, 53, 49, .12);
          ...
          .course-filter-chip {{
            ...
            border: 1px solid rgba(31, 53, 49, .18);
            color: var(--forest-strong);
          }}
          .course-filter-chip:hover {{
            transform: translateY(-2px);
            box-shadow: 0 10px 24px rgba(28, 31, 27, .12);
          }}
          .course-filter-chip.active {{
            color: #fff;
            background: linear-gradient(135deg, var(--accent), var(--accent-strong));
            border-color: transparent;
            box-shadow: 0 16px 28px rgba(204,84,50,.28);
          }}
          .course-filter-chip.active:hover {{
            transform: none;
            box-shadow: 0 16px 28px rgba(204,84,50,.28);
          }}
          .stat-grid {{
          ...
          /* The platform home is an entry lobby, so it gets a focused full-width canvas. */
          .layout:has(.home-shell) {{ grid-template-columns: 1fr; }}
          .layout:has(.home-shell) > .rail {{ display: none; }}
          .home-shell {{ display: grid; gap: 26px; }}
          .home-shell .home-hero {{
            ...
            color: #fffaf3;
            background:
              radial-gradient(circle at 76% 18%, rgba(238,180,91,.23), transparent 25%),
              radial-gradient(circle at 10% 92%, rgba(211,82,47,.2), transparent 28%),
              linear-gradient(132deg, #071f1c 0%, #113e38 52%, #172e2a 100%);
            box-shadow: 0 40px 100px rgba(7,31,28,.28);
          }}
          .home-shell .home-hero::before {{
            height: 1px;
            background: linear-gradient(90deg, transparent, rgba(255,230,190,.9), transparent);
          }}
          ...
          .home-shell .home-hero .hero-grid {{
            position: relative;
            z-index: 1;
          ...
          .home-shell .home-hero .home-lead {{
            max-width: 760px;
            color: rgba(255,248,238,.78);
            font-size: clamp(16px, 1.45vw, 20px);
            line-height: 1.85;
          }}
          .home-shell .home-hero .eyebrow {{ color: #efbf73; }}
          .home-shell .home-hero .hero-badge {{
          ...
          .home-shell .hero-panel h2 {{ color: #fffaf3; }}
          .home-shell .hero-panel > p {{ color: rgba(255,248,238,.68); }}
          .home-shell .command-card {{
            min-height: 145px;
            border: 1px solid rgba(255,255,255,.12);
            transition: transform .2s ease, background .2s ease, border-color .2s ease;
          }}
          .home-shell .command-card:hover {{
            transform: translateY(-4px);
            background: rgba(255,255,255,.13);
            border-color: rgba(239,191,115,.55);
          }}
          .home-shell .command-card strong {{ font-size: 18px; }}
          .home-shell .home-role-section {{
            padding: clamp(28px, 4vw, 52px);
            background: rgba(255,253,248,.88);
          }}
          .home-shell .home-role-section .card {{
            min-height: 260px;
            transition: transform .22s ease, box-shadow .22s ease;
          }}
          .home-shell .home-role-section .card:hover {{
            transform: translateY(-6px);
            box-shadow: 0 25px 55px rgba(14,42,37,.14);
          }}
          .home-shell .route-card {{
            min-height: 260px;
            transition: transform .22s ease, box-shadow .22s ease, border-color .22s ease;
          }}
          .home-shell .route-card:hover {{
            transform: translateY(-6px);
            border-color: rgba(204,84,50,.25);
            box-shadow: 0 25px 55px rgba(14,42,37,.13);
          }}
          /* Tiffany refresh: calm, bright, and readable across every portal. */
          :root {{
            --bg: #eef9f8;
            --bg-deep: #d9efed;
            --paper: rgba(255,255,255,.9);
            --paper-strong: #ffffff;
            --paper-muted: rgba(239,249,248,.94);
            --ink: #173b45;
            --muted: #52717a;
            --line: rgba(31,101,110,.13);
            --line-strong: rgba(31,101,110,.22);
            --accent: #3eaaa4;
            --accent-strong: #267f7b;
            --accent-soft: rgba(129,216,208,.2);
            --forest: #276b72;
            --forest-strong: #174e59;
            --forest-soft: rgba(129,216,208,.18);
            --gold: #74c9c3;
            --gold-soft: rgba(129,216,208,.2);
            --shadow-lg: 0 30px 76px rgba(42,105,111,.14);
            --shadow-md: 0 18px 42px rgba(42,105,111,.11);
            --shadow-sm: 0 12px 28px rgba(42,105,111,.08);
          }}
          body {{
            background:
              radial-gradient(circle at 8% 5%, rgba(129,216,208,.34), transparent 25%),
              radial-gradient(circle at 96% 2%, rgba(194,239,235,.72), transparent 30%),
              radial-gradient(circle at 85% 88%, rgba(125,194,210,.2), transparent 25%),
              linear-gradient(180deg, #f8fdfc 0%, #edf8f7 48%, #dfeeed 100%);
          }}
          .topbar {{
            background:
              linear-gradient(115deg, rgba(224,248,245,.98), rgba(153,224,217,.96) 52%, rgba(115,200,199,.95)),
              radial-gradient(circle at top right, rgba(255,255,255,.72), transparent 34%);
            border-color: rgba(255,255,255,.74);
            box-shadow: 0 22px 55px rgba(42,105,111,.16);
          }}
          .brand-mark {{
            color: #174e59;
            background: linear-gradient(145deg, #f5fffe, #81d8d0 58%, #5bbab6);
            box-shadow: 0 15px 30px rgba(42,105,111,.18);
          }}
          .brand-link, .brand-subtitle {{ color: #174e59; }}
          .brand-caption {{ color: rgba(23,78,89,.76); }}
          .signal-pill, .utility-pill, .lang-pill {{
            color: #174e59;
            background: rgba(255,255,255,.58);
            border-color: rgba(23,78,89,.13);
          }}
          .signal-pill:hover, .utility-pill:hover, .lang-pill:hover {{
            color: #0f424b;
            background: rgba(255,255,255,.88);
          }}
          .lang-label {{ color: rgba(23,78,89,.72); }}
          .lang-pill.active {{
            color: #174e59;
            background: rgba(255,255,255,.82);
            border-color: rgba(23,78,89,.2);
          }}
          .home-shell .home-hero {{
            color: #173b45;
            background:
              radial-gradient(circle at 79% 14%, rgba(255,255,255,.9), transparent 27%),
              radial-gradient(circle at 7% 94%, rgba(85,183,181,.19), transparent 28%),
              linear-gradient(132deg, #edfafa 0%, #bdebe7 52%, #8ddad4 100%);
            box-shadow: 0 38px 90px rgba(42,105,111,.2);
          }}
          .home-shell .home-hero h1 {{ color: #174e59; }}
          .home-shell .home-hero .home-lead {{ color: rgba(23,78,89,.78); }}
          .home-shell .home-hero .eyebrow {{ color: #267f7b; }}
          .home-shell .home-hero .hero-badge {{
            color: #174e59;
            background: rgba(255,255,255,.52);
            border-color: rgba(23,78,89,.11);
          }}
          .home-shell .hero-panel {{
            background: rgba(255,255,255,.42);
            border-color: rgba(255,255,255,.64);
            box-shadow: inset 0 1px rgba(255,255,255,.7), 0 20px 44px rgba(42,105,111,.1);
          }}
          .home-shell .hero-panel h2,
          .home-shell .command-card strong {{ color: #174e59; }}
          .home-shell .hero-panel > p,
          .home-shell .command-card p {{ color: rgba(23,78,89,.72); }}
          .home-shell .command-card {{
          ...
          .btn {{
            ...
            box-shadow: 0 14px 28px rgba(38,127,123,.2);
          }}
          .btn.alt {{ color: #174e59; }}
          .hero::before, .section::before, .card::before {{
            background: linear-gradient(90deg, #81d8d0, #bceee9, #5cb9bc);
          }}
          .eyebrow, .label, .route-label, .portal-nav-kicker, .journey-step,
          .lane-index, .rail-label, .hero-note-label, .command-card > span {{
            font-size: 13px;
          }}
          .brand-link, .brand-caption, .signal-pill, .lang-label,
          .chip, .rail-chip, .status, .subtle-badge, .route-card-foot,
          .task-marker, .course-filter-chip {{ font-size: 14px; }}
          .brand-subtitle, .nav-link-title, .utility-pill, .lang-pill,
          .task-content p, .note-card p, .schedule-line {{ font-size: 16px; }}
          .nav-link-note, .command-card p, .lane-note, .hero-note-detail,
          .portal-nav-card p {{ font-size: 15px; }}
          .home-shell > section {{ animation: home-rise .65s both; }}
          .home-shell > section:nth-child(2) {{ animation-delay: .08s; }}
          .home-shell > section:nth-child(3) {{ animation-delay: .14s; }}
          @keyframes home-rise {{
            from {{ opacity: 0; transform: translateY(18px); }}
            to {{ opacity: 1; transform: translateY(0); }}
          }}
          @media (prefers-reduced-motion: reduce) {{
            .home-shell > section {{ animation: none; }}
          }}
          .task-list {{
            display: grid;
            gap: 12px;
          ...
          @media (max-width: 900px) {{
            ...
            .hero-grid, .grid, .grid.two, .stat-grid, .feature-grid, .route-grid, .entry-quick-grid, .command-board {{ grid-template-columns: 1fr; }}
            .topbar {{
              align-items: flex-start;
          ...
          @media (max-width: 640px) {{
            ...
            .home-shell .home-hero {{ padding: 30px 20px; }}
      ...
      <body>
        ...
        <div class="topbar">
            ...
            <div class="topbar-actions">
              <div class="signal-strip">
                <a class="signal-pill" data-topbar-route="/school-platform/franchise-vap" href="/school-platform/franchise-vap">招商入口</a>
                <a class="signal-pill" data-topbar-route="/school-platform/student-portal?email=web-enrollment%40example.com" href="/school-platform/student-portal?email=web-enrollment%40example.com">學員教學</a>
                <a class="signal-pill" data-topbar-route="/school-platform/admin" href="/school-platform/admin">營運後台</a>
              </div>
              <a class="utility-pill" href="/school-platform/progress">開發進度</a>
              <a class="utility-pill" href="/school-platform/login">登入</a>
        ...
        <script>
          (() => {{
            ...
            document.querySelectorAll('.topbar .signal-pill[data-topbar-route]').forEach((node) => {{
              const fallbackHref = normalizeSchoolPlatformUrl(node.getAttribute('data-topbar-route') || node.getAttribute('href') || '');
              if (fallbackHref) {{
                node.setAttribute('href', fallbackHref);
              }}
            }});
...
def school_platform_login_page(
    next: str | None = Query(default="/school-platform/admin"),
    error: str | None = Query(default=None),
) -> str:
    next_url = _safe_platform_redirect(next)
    error_html = _login_error_message(error)
    body = f"""
      <section class="hero">
        <div class="eyebrow">Access Control</div>
        <h1>平台登入</h1>
        <p>請以本機或後台帳號登入後，前往營運、教務、教師與顧問頁面。</p>
        {error_html}
      </section>
      <section class="section">
        <div class="grid two">
          <article class="card">
            <h2>帳號登入</h2>
            <form method="post" action="/school-platform/login" class="stack">
              <input type="hidden" name="next" value="{escape(next_url)}" />
              <label class="field">
                Email
                <input name="email" type="email" required placeholder="admin@jls.local" />
              </label>
              <label class="field">
                Password
          ...
          <article class="card">
            ...
            <div class="helper-list">
              <div>目前環境可透過 seed 帳號登入。</div>
              <div>建議登錄成功後再用上方或左側導覽前往後台。</div>
              <div>若你已持有 access_token，也可改用 URL ?access_token=... 直接授權。</div>
            </div>
          </article>
        </div>
      </section>
    """
    return _page_shell("平台登入", body)


@router.post("/login")
def school_platform_login_submit(
    request: Request,
    email: str = Form(...),
    password: str = Form(...),
    next: str = Form(default="/school-platform/admin"),
) -> Response:
    try:
        session = auth_service.authenticate(email, password)
    except HTTPException as exc:
        return school_platform_login_page(next=next, error=exc.detail)
    return _build_login_cookie_response(next, session.access_token, request)
...
def school_platform_home() -> str:
    ...
    platform_entries = [
        ...
        {
            "label": "完整網址入口表",
            "path": "/school-platform/site-entries",
            "category": "Operations",
            "audience": "全體 / 業務 / 夥伴",
            "description": "把 platform 目前所有正式網址依群組、方法與路徑完整列出，供招募與跨團隊同步。",
        },
    ]
    hero_notes = [
        {
    ...
    <div class="home-shell">
      <section class="hero home-hero">
        <div class="hero-grid">
          <div>
            <div class="eyebrow">Japan Life Language School OS</div>
            <h1>從學會日語，<br>到走進日本生活。</h1>
            <p class="home-lead">一個把 AI 招生、真人教學、雙向即時翻譯、學員成長與校務營運連成一體的日語教育平台。選擇你的角色，直接開始今天的任務。</p>
            <div class="hero-badge-row">{hero_badge_html}</div>
            <div class="actions">
              <a class="btn" href="{student_portal_url}">我是學員</a>
              <a class="btn alt" href="{teacher_portal_url}">我是老師</a>
              <a class="btn alt" href="/school-platform/admin">我是營運人員</a>
              <a class="btn alt" href="/school-platform/franchise-vap">我想了解加盟</a>
            </div>
          ...
          <article class="hero-panel">
            ...
            <h2>現在要去哪裡？</h2>
            <p>四個主要入口已經依使用情境整理好，點選後即可進入示範工作區。</p>
            <div class="command-board">{quick_start_html}</div>
          </article>
        </div>
      </section>
      <section class="section home-role-section">
      ...
      <section class="section" id="entry-franchise">
        <div class="section-head">
          <div>
            <div class="eyebrow">核心入口一：招商入口</div>
            <h2>加盟與市場入口</h2>
          </div>
          <p class="section-subtitle">直接進入這三個頁面就能確認招商故事、權益設計與加盟報表狀況。</p>
        </div>
        <div class="entry-quick-grid">
          <article class="entry-quick-card">
            <div class="eyebrow">Franchise</div>
            <h3>加盟招商 VAP</h3>
            <p>展示 AI-aging 招商策略、十區規劃、加盟權益與價值主張，作為外部招商第一入口。</p>
            <div class="actions"><a class="btn" href="/school-platform/franchise-vap">前往加盟招商</a></div>
          </article>
          <article class="entry-quick-card">
            <div class="eyebrow">Franchise Data</div>
            <h3>加盟組報表</h3>
            <p>即時檢視招商漏斗、區域銷售進度與加盟收入狀態，給夥伴與主管同步現況。</p>
            <div class="actions"><a class="btn alt" href="/school-platform/admin/reports/franchise">查看加盟報表</a></div>
          </article>
          <article class="entry-quick-card">
            <div class="eyebrow">Growth Track</div>
            <h3>招生總覽入口</h3>
            <p>從公開招生入口到前台報名頁，先把招生線轉成可執行名單流程。</p>
            <div class="actions"><a class="btn alt" href="/school-platform/trial-booking">前往試聽預約</a></div>
          </article>
        </div>
      </section>
      <section class="section" id="entry-student">
        <div class="section-head">
          <div>
            <div class="eyebrow">核心入口二：學員教學</div>
            <h2>學員端 / 學習入口</h2>
          </div>
          <p class="section-subtitle">你可以直接從示範帳號快速進到學員中心與 AI 學習區。</p>
        </div>
        <div class="entry-quick-grid">
          <article class="entry-quick-card">
            <div class="eyebrow">Student Portal</div>
            <h3>學員中心</h3>
            <p>查看課表、作業、測驗、付款、通知與客服需求，直接確認學習流程是否順暢。</p>
            <div class="actions"><a class="btn" href="{student_portal_url}">進入學員中心</a></div>
          </article>
          <article class="entry-quick-card">
            <div class="eyebrow">AI Practice</div>
            <h3>AI 練習區</h3>
            <p>用對話任務帶學生做生活情境與職場情境練習，快速產生回饋訊息。</p>
            <div class="actions"><a class="btn alt" href="{student_ai_url}">進入 AI 練習區</a></div>
          </article>
          <article class="entry-quick-card">
            <div class="eyebrow">Teaching</div>
            <h3>教師工作台</h3>
            <p>檢查待批改作業、待批改測驗與班級進度，支援教師直接切入授課管理。</p>
            <div class="actions"><a class="btn alt" href="{teacher_portal_url}">進入教師工作台</a></div>
          </article>
        </div>
      </section>
      <section class="section" id="entry-ops">
        <div class="section-head">
          <div>
            <div class="eyebrow">核心入口三：營運後台</div>
            <h2>營運管理</h2>
          </div>
          <p class="section-subtitle">從首頁、名單、財務、訊息與報表切換到營運任務，做完整日常控制。</p>
        </div>
        <div class="entry-quick-grid">
          <article class="entry-quick-card">
            <div class="eyebrow">Operations</div>
            <h3>營運後台總覽</h3>
            <p>查看招生、教務、客服、AI 以及行政工作的最新整體快照。</p>
            <div class="actions"><a class="btn" href="/school-platform/admin">開啟營運後台</a></div>
          </article>
          <article class="entry-quick-card">
            <div class="eyebrow">Teaching Ops</div>
            <h3>教務管理</h3>
            <p>班級、作業、測驗、點名與教師回饋集中管理。</p>
            <div class="actions"><a class="btn alt" href="/school-platform/admin/teaching">前往教務管理</a></div>
          </article>
          <article class="entry-quick-card">
            <div class="eyebrow">Finance</div>
            <h3>財務中心</h3>
            <p>確認報名、付款、待收款、收支與對帳節奏，快速掌握營收狀態。</p>
            <div class="actions"><a class="btn alt" href="/school-platform/admin/finance">開啟財務中心</a></div>
          </article>
        </div>
      </section>
      <section class="section">
...
def school_platform_courses_page(focus: str = Query(default="all")) -> str:
    courses = catalog_service.list_courses()

    focus_key = (focus or "all").strip().lower()
    if focus_key not in {"all", "life", "work", "jlpt"}:
        focus_key = "all"

    grouped_courses: dict[str, list] = {"life": [], "work": [], "jlpt": [], "other": []}
    for course in courses:
        grouped_courses[_course_scenario_key(course.course_type)].append(course)
    ...
    focus_options = [
        ("all", "全部", len(courses)),
        ("life", "生活日語", len(grouped_courses["life"])),
        ("work", "職場日語", len(grouped_courses["work"])),
        ("jlpt", "JLPT", len(grouped_courses["jlpt"])),
    ]
    filter_chips = "".join(
        (
            f"<a class='course-filter-chip{' active' if key == focus_key else ''}' "
            f"href='/school-platform/courses{'' if key == 'all' else f'?focus={key}'}'>"
            f"{escape(label)} ({count})</a>"
        )
        for key, label, count in focus_options
    )

    def render_card(course, scenario: str) -> str:
        return (
            "<article class='card' data-scenario='{scenario}'>"
            f"<div class='eyebrow'>{escape(course.course_type)} / {escape(course.level)}</div>"
            f"<h3>{escape(course.name)}</h3>"
            f"<p>{escape(course.short_description)}</p>"
            "<div class='meta'>"
            ...
            f"<div class='actions'><a class='btn' href='/school-platform/courses/{escape(course.slug)}'>查看課程詳情</a></div>"
            "<div class='meta'>"
            "<span class='chip'>報名後將進入學員中心</span>"
            "<span class='chip'>開課後可看課表與通知</span>"
            "</div>"
            f"<div class='actions'>"
            f"<a class='btn alt' href='/school-platform/courses/{escape(course.slug)}?preview=student-entry'>看學生入口示意</a>"
            f"<a class='btn alt' href='/school-platform/trial-booking?course_slug={escape(course.slug)}'>預約試聽</a>"
            "</div>"
            "</article>"
        ).replace("{scenario}", escape(scenario))

    group_headings = [
        ("life", "職涯場景導向／生活日語", "生活日語課程"),
        ("work", "職涯場景導向／職場日語", "職場日語課程"),
        ("jlpt", "職涯場景導向／JLPT", "JLPT 課程"),
    ]
    if focus_key != "all":
        group_headings = [
            (
                focus_key,
                f"職涯場景導向／{focus_label_by_key.get(focus_key, focus)}",
                f"{focus_label_by_key.get(focus_key, focus)} 課程",
            )
        ]

    grouped_sections = []
    for key, heading, description in group_headings:
        items = grouped_courses[key]
        cards = "".join(render_card(course, key) for course in items)
        if not cards:
            cards = "<article class='card'><h3>目前沒有課程</h3><p>此場景區域尚未建立課程，請先新增課程。</p></article>"
        grouped_sections.append(
            ...
            <section class="section">
              <div class="section-head">
                ...
                <p class="section-subtitle">以下為「{escape(heading)}」的課程，可直接複製到招生對話。</p>
              </div>
    ...
    cards_by_focus = "".join(grouped_sections)
    ...
    body = f"""
      <section class="hero">
        ...
        <div class="course-filter-row">
          <div class="course-filter-label">職涯場景導向</div>
          <div class="course-filter-chips">{filter_chips}</div>
        </div>
        <div class="actions">
...
def school_platform_course_detail_page(slug: str) -> str:
    ...
    classes = sorted(
        catalog_service.classes_for_course(slug),
        key=lambda item: (item.start_date, item.start_time, item.name),
    )
    class_ids = {item.id for item in classes}
    active_enrollments = [
        item
        for item in store.list_enrollments()
        if item.class_id in class_ids and item.status in {"pending", "active"}
    ]
    preview_enrollment = max(active_enrollments, key=lambda item: item.created_at) if active_enrollments else None
    preview_student = store.get_student_by_id(preview_enrollment.student_id) if preview_enrollment else None
    preview_email = preview_student.email if preview_student else None
    next_class = classes[0] if classes else None
    next_class_window = (
        f"{next_class.weekday} {next_class.start_time.strftime('%H:%M')}-{next_class.end_time.strftime('%H:%M')}"
        if next_class
        else "報名確認後自動帶入"
    )
    next_class_period = (
        f"{next_class.start_date.isoformat()} 至 {next_class.end_date.isoformat()}"
        if next_class
        else "依開班梯次安排"
    )
    student_preview_points = "".join(
        f"<div class='data-point'><span>{escape(label)}</span><strong>{escape(value)}</strong></div>"
        for label, value in [
            ("預計入班", next_class.name if next_class else "顧問確認後分班"),
            ("上課節奏", next_class_window),
            ("上課地點", next_class.location_label if next_class else course.delivery_mode),
            ("教材層次", f"平台 {len(snapshot.platform_materials)} 份 / 教師 {len(snapshot.teacher_materials)} 份"),
        ]
    )
    student_preview_tasks = _render_task_items(
        [
            {
                "index": "01",
                "title": "先看到班級時段與老師",
                "note": (
                    f"右側先直接顯示 {next_class_window}、授課老師 {next_class.teacher_name} 與開課區間，讓學員知道自己會被分到哪一梯。"
                    if next_class
                    else "若目前還沒有公開班級，右側會先顯示待開班與顧問安排中的提示。"
                ),
            },
            {
                "index": "02",
                "title": "報名後自動進我的課表",
                "note": (
                    "一旦完成報名與付款，班級會自動進入學員的我的課表，若是 Zoom 班也會同步出現進入教室按鈕。"
                    if next_class and is_zoom_delivery(next_class.location_label, next_class.meeting_url)
                    else "一旦完成報名與付款，班級會自動進入學員的我的課表與通知中心。"
                ),
            },
            {
                "index": "03",
                "title": "開課後持續看教材、通知與歷程",
                "note": "開課後不只看時段，還會從通知中心收到提醒，並在我的歷程追蹤作業、測驗、出缺勤與 AI 練習。",
            },
        ]
    )
    student_preview_actions = [
        f"<a class='btn' href='/school-platform/trial-booking?course_slug={escape(course.slug)}'>預約這門課試聽</a>",
        f"<a class='btn alt' href='/school-platform/enrollment?course_slug={escape(course.slug)}'>直接報名這門課</a>",
    ]
    if preview_email:
        student_preview_actions.extend(
            [
                f"<a class='btn alt' href='/school-platform/student-portal?email={escape(preview_email)}'>打開學員中心預覽</a>",
                f"<a class='btn alt' href='/school-platform/my-schedule?email={escape(preview_email)}'>看報名後課表</a>",
            ]
        )
    if next_class and next_class.meeting_url:
        student_preview_actions.append(_meeting_entry_button(next_class.meeting_url, label="模擬進入同步課"))
    student_preview_nav = _render_portal_nav_cards(
        [
            {
                "kicker": "Portal",
                "title": "學員中心",
                "note": "報名完成後的主入口，先看目前綁定的班級、付款狀態與待處理項目。",
                "href": (
                    f"/school-platform/student-portal?email={escape(preview_email)}"
                    if preview_email
                    else f"/school-platform/enrollment?course_slug={escape(course.slug)}"
                ),
            },
            {
                "kicker": "Schedule",
                "title": "我的課表",
                "note": (
                    f"會顯示 {next_class_window}、地點、老師與開課區間，若是 Zoom 班就能直接點進教室。"
                    if next_class
                    else "完成分班後，這裡會顯示學員實際的上課時間與地點。"
                ),
                "href": (
                    f"/school-platform/my-schedule?email={escape(preview_email)}"
                    if preview_email
                    else f"/school-platform/trial-booking?course_slug={escape(course.slug)}"
                ),
            },
            {
                "kicker": "Notice",
                "title": "通知中心",
                "note": "開課提醒、調課、教材發布與繳費提醒都會集中在這裡，不用再分散到聊天紀錄裡找。",
                "href": (
                    f"/school-platform/notifications-center?email={escape(preview_email)}"
                    if preview_email
                    else f"/school-platform/enrollment?course_slug={escape(course.slug)}"
                ),
            },
            {
                "kicker": "Progress",
                "title": "我的歷程",
                "note": "上課後會追蹤作業、測驗、出缺勤與 AI 練習的完成狀態，讓學員知道自己進度在哪裡。",
                "href": (
                    f"/school-platform/my-history?email={escape(preview_email)}"
                    if preview_email
                    else f"/school-platform/enrollment?course_slug={escape(course.slug)}"
                ),
            },
    ...
    experience_cards = "".join(
        [
            (
                ...
                "<div class='eyebrow'>Before Enrollment</div>"
                "<h3>報名前先確認這門課適不適合自己</h3>"
                f"<p>學員會先看到課程定位、價格、授課方式、老師與目前開放的班級時段，並可直接預約試聽或報名。</p>"
                f"<div class='meta'><span class='chip'>{_format_jpy(course.price)}</span><span class='chip'>{escape(course.delivery_mode)}</span></div>"
                "</article>"
            ),
            (
                "<article class='focus-card'>"
                "<div class='eyebrow'>After Enrollment</div>"
                "<h3>報名後右側最關鍵的是班級資訊</h3>"
                f"<p>真正進入學員端後，最先被確認的是班級名稱、老師、上課時間、地點與 {'Zoom 入口' if next_class and next_class.meeting_url else '同步上課方式'}，不會只剩抽象課名。</p>"
                f"<div class='meta'><span class='chip'>{escape(next_class.name) if next_class else '待分班'}</span><span class='chip'>{escape(next_class_window)}</span></div>"
                "</article>"
            ),
            (
                "<article class='focus-card'>"
                "<div class='eyebrow'>In Class</div>"
                "<h3>開課後再往下接教材、通知與學習記錄</h3>"
                f"<p>平台自有教材 {len(snapshot.platform_materials)} 份、教師補充 {len(snapshot.teacher_materials)} 份，之後會一路串到通知中心、作業、測驗與我的歷程。</p>"
                f"<div class='meta'><span class='chip'>核心章節 {len(snapshot.core_modules) or len(course.modules)}</span><span class='chip'>內容已分層治理</span></div>"
                "</article>"
            ),
        ]
    )
    class_cards = "".join(
        ...
        f"<div class='meta'><span class='chip'>名額 {class_item.enrolled_count}/{class_item.capacity}</span><span class='chip'>{escape(class_item.location_label)}</span>{_meeting_status_chip(class_item.location_label, class_item.meeting_url)}</div>"
        f"<div class='actions'><a class='btn alt' href='/school-platform/trial-booking?course_slug={escape(course.slug)}'>預約試聽</a>{_meeting_entry_button(class_item.meeting_url)}</div>"
        "</article>"
        ...
        <div class="workspace-hero-grid">
          <div class="workspace-copy">
            ...
            <div class="meta">
              ...
              <span class="chip">開班 {len(classes)} 梯</span>
            </div>
            <div class="actions">
          ...
          <article class="workspace-panel">
            <div class="eyebrow">Student Preview</div>
            <h2>學員選定課程後，右側會看到這些內容</h2>
            <div class="data-points">{student_preview_points}</div>
            <div class="task-list">{student_preview_tasks}</div>
            <div class="actions">{''.join(student_preview_actions)}</div>
          </article>
        </div>
      </section>
      <section class="section">
        <div class="section-head">
          <div>
            <div class="eyebrow">Student Entry Preview</div>
            <h2>報名後入口預覽</h2>
          </div>
          <p class="section-subtitle">這一區直接告訴學員與業務，報名之後不是只剩一個課名，而是會進到課表、通知、歷程與學員中心這些實際入口。</p>
        </div>
        <div class="portal-nav-grid">{student_preview_nav}</div>
      </section>
      <section class="section">
        <div class="section-head">
          <div>
            <div class="eyebrow">Enrollment Journey</div>
            <h2>未來實際顯示情境</h2>
          </div>
          <p class="section-subtitle">我把報名前、報名後、開課中三段都具體放出來，之後你向學員或加盟主說明時會更直觀。</p>
        </div>
        <div class="focus-grid">{experience_cards}</div>
      </section>
      <section class="section">
        <div class="grid two">
          <article class="card">
...
def school_platform_material_download(
    ...
    if material.material_url:
        safe_url = _normalize_material_download_url(material.material_url)
        if not safe_url:
            raise HTTPException(status_code=403, detail="Material URL is blocked")
        return RedirectResponse(url=safe_url, status_code=status.HTTP_307_TEMPORARY_REDIRECT)
...
def school_platform_student_portal_page(email: str = Query(...)) -> str:
    ...
    class_cards = "".join(
        ...
        f"<div class='meta'><span class='chip'>{escape(item.start_date.isoformat())}</span><span class='chip'>{escape(item.teacher_name)}</span><span class='chip'>{item.enrolled_count}/{item.capacity}</span>{_meeting_status_chip(item.location_label, item.meeting_url)}</div>"
        f"<div class='actions'><a class='btn alt' href='/school-platform/my-schedule?email={escape(email)}'>查看課表</a>{_meeting_entry_button(item.meeting_url)}</div>"
        "</article>"
        for item in classes
...
def school_platform_student_schedule_page(email: str = Query(...)) -> str:
    ...
    cards = "".join(
        ...
        f"<div class='schedule-meta'><span class='chip'>{escape(item.location_label)}</span><span class='chip'>{escape(item.teacher_name)}</span><span class='chip'>{item.enrolled_count}/{item.capacity}</span>{_meeting_status_chip(item.location_label, item.meeting_url)}</div>"
        f"<div class='actions'>{_meeting_entry_button(item.meeting_url, css_class='btn')}</div>"
        "</article>"
      ...
      <section class="section">
        <div class="grid two">
          ...
          <article class="card">
            ...
            <p>可直接跑 <code>python3 scripts/verify_school_platform_notification_delivery.py --channels email --require-sent</code> 做一鍵 API 驗證。</p>
          </article>
          <article class="card">
            <h2>LINE 外發需要什麼</h2>
            ...
            <p>舊命名也可用：<code>LINE_CHANNEL_ACCESS_TOKEN</code>、<code>LINE_CHANNEL_SECRET</code>、<code>LINE_FALLBACK_USER_ID</code></p>
            <p>補完後可用 <code>python3 scripts/test_school_platform_notification_providers.py --channel line --recipient Uxxxxxxxx...</code> 驗證。</p>
            <p>完整 API 流程可用 <code>python3 scripts/verify_school_platform_notification_delivery.py --channels line --require-sent</code> 驗證。</p>
          </article>
        </div>
      </section>
      <section class="section">
...
def school_platform_admin_classes_page() -> str:
    ...
    class_cards = "".join(
        ...
        f"{_meeting_status_chip(item.location_label, item.meeting_url)}"
        f"<span class='chip'>{escape(item.status)}</span>"
        "</div>"
        f"<div class='actions'><a class='btn' href='/school-platform/admin/classes/{item.id}/edit'>編輯班級</a>{_meeting_entry_button(item.meeting_url)}</div>"
        "</article>"
        for item in classes
        ...
        <div class="grid two">
          <article class="card">
            ...
            <form class="stack" method="post" action="/school-platform/admin/classes/create">
              ...
              <label class="field">Zoom 會議連結
                <input type="url" name="meeting_url" placeholder="https://us06web.zoom.us/j/..." />
              </label>
              <p>如果這堂課是 Zoom 同步上課，請直接貼上 Zoom 連結；若未填入且 Zoom API 已設定，系統會自動建立會議。</p>
      ...
      <section class="section">
        <article class="card">
          <form class="stack" method="post" action="/school-platform/admin/classes/{class_item.id}/edit">
            ...
            <label class="field">Zoom 會議連結
              <input type="url" name="meeting_url" value="{escape(class_item.meeting_url or '')}" placeholder="https://us06web.zoom.us/j/..." />
            </label>
            <p>若這堂課維持 Zoom 上課，請保留或更新 meeting_url；若留空且 Zoom API 已設定，系統會自動補建。</p>
...
def school_platform_admin_courses_page(
    package: str = Query(default="all"),
    level: str = Query(default="all"),
) -> str:
    courses = catalog_service.list_courses()
    package_key = (package or "all").strip().lower()
    if package_key not in {"all", "life", "work", "jlpt", "other"}:
        package_key = "all"
    level_key = (level or "all").strip().lower()

    level_counts: dict[str, int] = {}
    for item in courses:
        key = _course_level_key(item.level)
        level_counts[key] = level_counts.get(key, 0) + 1

    package_counts: dict[str, int] = {"all": len(courses), "life": 0, "work": 0, "jlpt": 0, "other": 0}
    for item in courses:
        package_counts[_course_scenario_key(item.course_type)] += 1

    def _build_admin_course_filter_url(package_filter: str, level_filter: str) -> str:
        query: dict[str, str] = {}
        if package_filter != "all":
            query["package"] = package_filter
        if level_filter != "all":
            query["level"] = level_filter
        if not query:
            return "/school-platform/admin/courses"
        return f"/school-platform/admin/courses?{urlencode(query)}"

    package_options = [
        ("all", "全部課程包", package_counts["all"]),
        ("life", "生活日語課程包", package_counts["life"]),
        ("work", "職場日語課程包", package_counts["work"]),
        ("jlpt", "JLPT 課程包", package_counts["jlpt"]),
        ("other", "其他課程包", package_counts["other"]),
    ]
    package_chips = "".join(
        (
            f"<a class='course-filter-chip{' active' if key == package_key else ''}' "
            f"href='{escape(_build_admin_course_filter_url(key, level_key))}'>"
            f"{escape(label)} ({count})</a>"
        )
        for key, label, count in package_options
    )

    level_keys = sorted(
        (item for item in level_counts.keys() if item != "all"),
        key=lambda item: item.lower(),
    )
    level_options = [("all", "全部難度", len(courses))]
    level_options.extend(
        (
            key,
            key.upper() if key.startswith("n") else (key if key else "未設定"),
            level_counts.get(key, 0),
        )
    ...
    level_chips = "".join(
        (
            f"<a class='course-filter-chip{' active' if key == level_key else ''}' "
            f"href='{escape(_build_admin_course_filter_url(package_key, key))}'>"
            f"{escape(label)} ({count})</a>"
        )
        for key, label, count in level_options
    )

    def render_admin_course_card(course) -> str:
        scenario_label = _course_scenario_label(course.course_type)
        package_label = (
            "生活日語課程包" if scenario_label == "生活日語"
            else "職場日語課程包" if scenario_label == "職場日語"
            else "JLPT 課程包" if scenario_label == "JLPT"
            else "其他課程包"
        )
        level_label = course.level or "未設定"
        level_display = level_label.upper() if level_label.lower().startswith("n") else level_label
        return (
            "<article class='card'>"
            f"<div class='eyebrow'>{escape(course.course_type)} / {escape(course.level)}</div>"
            f"<h3>{escape(course.name)}</h3>"
            ...
            f"<span class='chip'>{_format_jpy(course.price)}</span>"
            f"<span class='chip'>{escape(course.delivery_mode)}</span>"
            f"<span class='chip'>場景：{escape(scenario_label)}</span>"
            f"<span class='chip'>{escape(package_label)}</span>"
            f"<span class='chip'>難度：{escape(level_display)}</span>"
            "</div>"
            f"<div class='actions'><a class='btn' href='/school-platform/admin/courses/{escape(course.slug)}/edit'>編輯課程</a></div>"
            "</article>"
        )
    ...
    filtered_courses = [
        item
        for item in courses
        if (package_key == "all" or _course_scenario_key(item.course_type) == package_key)
        and (level_key == "all" or _course_level_key(item.level) == level_key)
    ]
    course_cards = "".join(render_admin_course_card(item) for item in filtered_courses) or "<article class='card'><h3>目前沒有課程</h3></article>"
    body = f"""
      <section class="hero">
        ...
        <div class="course-filter-row">
          <div class="course-filter-label">課程包</div>
          <div class="course-filter-chips">{package_chips}</div>
        </div>
        <div class="course-filter-row">
          <div class="course-filter-label">難度</div>
          <div class="course-filter-chips">{level_chips}</div>
        </div>
        <div class="actions">
          <a class="btn" href="/school-platform/admin">回營運總覽</a>
...
def school_platform_teacher_portal_page(teacher_name: str = Query(...)) -> str:
    ...
    class_cards = "".join(
        ...
        f"<h3>{escape(item.name)}</h3>"
        f"<p>{escape(item.weekday)} / {escape(item.start_time.strftime('%H:%M'))}-{escape(item.end_time.strftime('%H:%M'))}</p>"
        f"<div class='meta'><span class='chip'>{escape(item.location_label)}</span><span class='chip'>{item.enrolled_count}/{item.capacity}</span>{_meeting_status_chip(item.location_label, item.meeting_url)}</div>"
        f"<div class='actions'><a class='btn' href='/school-platform/teacher/classes/{item.id}?teacher_name={escape(teacher_name)}'>查看班級詳情</a>{_meeting_entry_button(item.meeting_url)}</div>"
        "</article>"
        for item in classes
        ...
        <div class="workspace-hero-grid">
          <div class="workspace-copy">
            ...
            <div class="meta">
              ...
              {_meeting_status_chip(class_item.location_label, class_item.meeting_url)}
            ...
            <div class="actions">
              ...
              {_meeting_entry_button(class_item.meeting_url)}
...
def school_platform_admin_ai_teaching_page(
    ...
    source_text: str | None = Query(default=None),
) -> str:
    classes = catalog_service.open_classes()
    class_options = "".join(
    ...
    translation = None
    selected_source_text = (source_text or "").strip()
    if selected_source_text and selected_class_id:
        try:
            translation = ai_assistant_service.bidirectional_translation(
                BiDirectionalTranslationRequest(
            ...
            translation = None
    translation_step_cards = (
        "".join(
            (
                "<article class='card'>"
                f"<div class='eyebrow'>{escape(item)}</div>"
                f"<h3>{escape(label)}</h3>"
                "</article>"
            )
            for item, label in [
                (translation.source_language, "來源語言"),
                (translation.target_language, "目標語言"),
                (translation.forward_translation, "正向翻譯"),
                (translation.reverse_translation, "反向翻譯"),
            ]
        )
        if translation
        else "<article class='card'><h3>輸入一句中文或日文，讓系統產生雙向翻譯與教學提示。</h3></article>"
    )
    teaching_notes = "".join(f"<li>{escape(item)}</li>" for item in translation.teaching_notes) if translation else "<li>先輸入原句</li>"
    classroom_prompts = "".join(f"<li>{escape(item)}</li>" for item in translation.classroom_prompts) if translation else "<li>先輸入原句</li>"
    review_checks = "".join(f"<li>{escape(item)}</li>" for item in translation.review_checks) if translation else "<li>先輸入原句</li>"
    body = f"""
      <section class="hero">
        <div class="eyebrow">AI Teaching Drafts</div>
        <h1>AI 教案草稿中心</h1>
        <p>這裡可以依照班級、課堂主題與時長，快速生成可給老師修改的教案草稿與課後作業建議，並同步提供雙向即時翻譯教學卡。</p>
        <div class="actions">
      ...
      <section class="section">
        <div class="section-head">
          <div>
            <div class="eyebrow">Bidirectional Translation</div>
            <h2>雙向即時翻譯教學卡</h2>
          </div>
          <p class="section-subtitle">把中文與日文互譯直接變成課堂使用的教材、提示與回收點。</p>
        </div>
        <div class="grid two">
          <article class="card">
            <h2>輸入原句</h2>
            <form class="stack" method="get" action="/school-platform/admin/ai-teaching">
              ...
              <label class="field">原句（中文或日文）
                <textarea name="source_text" rows="4" placeholder="例如：請幫我預約明天的試聽課。">{escape(selected_source_text)}</textarea>
              </label>
              <label class="field">課程時長（分鐘）
                <input type="number" name="duration_minutes" min="30" step="10" value="{duration_minutes}" />
              </label>
              <button class="btn" type="submit">生成雙向翻譯</button>
            </form>
          </article>
          <article class="card">
            <h2>翻譯結果摘要</h2>
            {f"<p>來源：{escape(translation.source_language)}</p><p>目標：{escape(translation.target_language)}</p><p>原句：{escape(translation.source_text)}</p><p>正向翻譯：{escape(translation.forward_translation)}</p><p>反向翻譯：{escape(translation.reverse_translation)}</p>" if translation else "<p>輸入原句後即可顯示雙向翻譯結果。</p>"}
          </article>
        </div>
        <div class="grid two">{translation_step_cards}</div>
        <div class="grid three">
          <article class="card"><h3>教學提示</h3><ul class="clean">{teaching_notes}</ul></article>
          <article class="card"><h3>課堂帶法</h3><ul class="clean">{classroom_prompts}</ul></article>
          <article class="card"><h3>回收檢查</h3><ul class="clean">{review_checks}</ul></article>
        </div>
      </section>
    """
...
def school_platform_admin_interview_create_submit(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="Applicant not found") from exc
    redirect_to = _safe_local_redirect(return_to) or "/school-platform/admin/recruiting"
...
async def school_platform_teacher_verification_submit(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    redirect_to = _safe_local_redirect(return_to) or f"/school-platform/teacher/verification?{urlencode({'teacher_name': teacher_name})}"
    return RedirectResponse(url=redirect_to, status_code=status.HTTP_303_SEE_OTHER)
...
def school_platform_teacher_assignment_grade_submit(
    ...
    redirect_to = _safe_local_redirect(return_to) or f"/school-platform/teacher-portal?{urlencode({'teacher_name': teacher_name})}"
    return RedirectResponse(url=redirect_to, status_code=status.HTTP_303_SEE_OTHER)
...
def school_platform_teacher_session_record_submit(
    ...
    redirect_to = _safe_local_redirect(return_to) or f"/school-platform/teacher/classes/{class_id}?{urlencode({'teacher_name': teacher_name})}"
    return RedirectResponse(url=redirect_to, status_code=status.HTTP_303_SEE_OTHER)
...
def school_platform_progress_page() -> str:
    ...
    route_cards = "".join(
        [
            ...
            "</article>",
            "<article class='route-card'>"
            "<div class='route-label'>系統入口總表</div>"
            "<code>/school-platform/site-entries</code>"
            "<p>將所有系統路由與 API 入口集中列出，方便外部窗口快速導覽。</p>"
            "</article>",
        ]
    )
    legend = "".join(
...
@router.get("/site-entries", response_class=HTMLResponse)
def school_platform_site_entries_page(
    group: str = Query(default=""),
    method: str = Query(default=""),
) -> str:
    selected_group = group.strip() or None
    selected_method = method.strip().upper() or None
    all_entries = _load_site_entry_rows()
    entries = _load_site_entry_rows(group_filter=selected_group, method_filter=selected_method)
    if not all_entries:
        body = """
          <section class="hero">
            <div class="eyebrow">System URLs</div>
            <h1>完整網址入口表</h1>
            <p>目前沒有找到網址索引檔，請先確認 docs/url-index/all_app_urls.csv 是否存在。</p>
          </section>
        """
        return _page_shell("完整網址入口表", body)
    ...
    ordered_groups = sorted(group_counts.items())
    ordered_methods = sorted(method_counts.items())

    def _filter_link(next_group: str | None = None, next_method: str | None = None) -> str:
        query: dict[str, str] = {}
        if next_group:
            query["group"] = next_group
        if next_method:
            query["method"] = next_method
        return "/school-platform/site-entries" + (f"?{urlencode(query)}" if query else "")

    group_filter_chips = "".join(
        "<a class='filter-chip{active}' href='{href}'>"
        "{label} ({count})"
        "</a>".format(
            active=" active" if ((selected_group is None and group_value is None) or (selected_group == group_value)) else "",
            href=escape(_filter_link(group_value, None)),
            label=escape(group_label),
            count=count,
        )
        for group_value, group_label, count in [
            (None, "全部群組", len(all_entries)),
            *((item, item, count) for item, count in ordered_groups),
        ]
    )
    method_filter_chips = "".join(
        "<a class='filter-chip{active}' href='{href}'>"
        "{label} ({count})"
        "</a>".format(
            active=" active" if ((selected_method is None and method_value is None) or (selected_method == method_value)) else "",
            href=escape(_filter_link(None, method_value)),
            label=escape(method_label),
            count=count,
        )
        for method_value, method_label, count in [
            (None, "全部方法", len(all_entries)),
            *((item, item, count) for item, count in ordered_methods),
        ]
    )

    grouped_entries: dict[str, list[dict[str, str]]] = {}
    for item in entries:
        grouped_entries.setdefault(item["group"], []).append(item)

    if not grouped_entries:
        grouped_sections = "<article class='card'><h3>目前沒有符合條件的條目</h3></article>"
    else:
        grouped_sections_parts: list[str] = []
        for group_key in sorted(grouped_entries.keys()):
            group_items = grouped_entries[group_key]
            rows_html = "".join(
                "<tr>"
                f"<td class='mono'><span class='chip'>{escape(item['method'])}</span></td>"
                f"<td class='mono'>{escape(item['path'])}</td>"
                f"<td>{'<a href=\"' + escape(item['url']) + '\" target=\"_blank\" rel=\"noreferrer noopener\">' + escape(item['url']) + '</a>'}</td>"
                "</tr>"
                for item in group_items
            )
            grouped_sections_parts.append(
                f"""
                <section class="section">
                  <div class="entry-section-head">群組：{escape(group_key)}（{len(group_items)} 筆）</div>
        ...
        grouped_sections = "".join(grouped_sections_parts)

    body = f"""
      <section class="hero">
        <div class="eyebrow">System Entry Map</div>
        <h1>完整網址入口表</h1>
        <p>這是目前平台與 API 全路由總表，已納入系統 docs 與各工作流入口，方便業務、加盟主、工程統一參考。</p>
        <div class="actions">
          <a class="btn" href="/school-platform/progress">回平台開發進度</a>
          <a class="btn alt" href="/school-platform/api/system/site-entries">下載 JSON 清單</a>
        </div>
      </section>
      <section class="section">
        <div class="site-entry-toolbar">
          <span class="label">群組篩選</span>
          <div class="chip-row">
            {group_filter_chips}
          </div>
        </div>
        <div class="site-entry-toolbar">
          <span class="label">方法篩選</span>
          <div class="chip-row">
            {method_filter_chips}
          </div>
        </div>
        <div class="stat-grid" style="margin: 16px 0 0;">
          <div class="stat">
            <div class="label">總條目</div>
            <div class="value">{len(entries)}</div>
          </div>
          <div class="stat">
            <div class="label">已套用群組</div>
            <div class="value">{escape(selected_group or "全部")}</div>
          </div>
          <div class="stat">
            <div class="label">已套用方法</div>
            <div class="value">{escape(selected_method or "全部")}</div>
          </div>
        </div>
      </section>
      {grouped_sections}
...
def school_platform_system_page() -> str:
    ...
    zoom_provider = summary["zoom_provider"]
    live_class_delivery = summary["live_class_delivery"]
    ...
    body = f"""
      ...
      <section class="section">
        ...
        <div class="grid two">
          ...
          <article class="card">
            <h3>Zoom Live Classes</h3>
            <p>provider：<code>{escape(str(zoom_provider['provider']))}</code></p>
            <p>mode：<code>{escape(str(zoom_provider['mode']))}</code></p>
            <p>ready：<code>{escape(str(zoom_provider['ready']).lower())}</code></p>
            <p>zoom_class_count：<code>{escape(str(live_class_delivery['zoom_class_count']))}</code></p>
            <p>zoom_classes_with_links：<code>{escape(str(live_class_delivery['zoom_classes_with_links']))}</code></p>
            <p>zoom_classes_missing_links：<code>{escape(str(live_class_delivery['zoom_classes_missing_links']))}</code></p>
            <p>{escape(str(live_class_delivery['message']))}</p>
          </article>
        </div>
      </section>
      <section class="section">
...
@api_router.get("/system/site-entries")
def school_platform_site_entries_api(
    group: str = Query(default=""),
    method: str = Query(default=""),
) -> dict[str, object]:
    selected_group = group.strip() or None
    selected_method = method.strip().upper() or None
    all_entries = _load_site_entry_rows()
    filtered_entries = _load_site_entry_rows(group_filter=selected_group, method_filter=selected_method)
    group_counts: dict[str, int] = {}
    method_counts: dict[str, int] = {}
    for item in all_entries:
        group_counts[item["group"]] = group_counts.get(item["group"], 0) + 1
...
@api_router.post("/messages/smoke-test")
def messages_smoke_test(
    payload: NotificationSmokeTestRequest,
    user=Depends(require_roles("super_admin", "manager", "consultant")),
) -> dict[str, object]:
    try:
        notification = notification_service.send_test_notification(
...
@api_router.post("/ai/bidirectional-translation")
def ai_bidirectional_translation(
    payload: BiDirectionalTranslationRequest,
    user=Depends(require_roles("super_admin", "manager", "consultant", "teacher")),
) -> dict[str, object]:
    try:
        return {"data": ai_assistant_service.bidirectional_translation(payload).model_dump(mode="json"), "error": None}
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Class not found") from exc
