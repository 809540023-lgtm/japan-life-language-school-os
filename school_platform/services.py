...
from school_platform.zoom_runtime import SchoolPlatformZoomRuntime, is_zoom_delivery, is_zoom_meeting_url
from school_platform.schemas import (
    ...
    BiDirectionalTranslationResponse,
...
class CurriculumAdminService:
    def __init__(self, store) -> None:
        self.store = store
        self.zoom_runtime = SchoolPlatformZoomRuntime(store.settings)

    def _is_local_demo_mode(self) -> bool:
        app_base_url = str(self.store.settings.app_base_url)
        return "127.0.0.1" in app_base_url or "localhost" in app_base_url

    @staticmethod
    def _local_demo_zoom_url(payload: ClassUpsertRequest) -> str:
        digest = hashlib.sha1(f"{payload.course_slug}:{payload.name}:{payload.weekday}".encode("utf-8")).hexdigest()
        numeric = str(int(digest[:14], 16))
        meeting_id = numeric[:11].zfill(11)
        return f"https://zoom.us/j/{meeting_id}?pwd=demo{meeting_id[-4:]}"

    @staticmethod
    def _normalize_meeting_url(raw_value: str | None) -> str | None:
        value = (raw_value or "").strip()
        if not value:
            return None
        if not re.match(r"^(https?://|zoommtg://)", value, flags=re.IGNORECASE):
            raise ValueError("Zoom 連結必須以 http://、https:// 或 zoommtg:// 開頭。")
        return value

    @staticmethod
    def _duration_minutes(payload: ClassUpsertRequest) -> int:
        start_dt = datetime.combine(payload.start_date, payload.start_time)
        end_dt = datetime.combine(payload.start_date, payload.end_time)
        duration = int((end_dt - start_dt).total_seconds() // 60)
        return duration if duration > 0 else 90

    def _resolve_class_payload(
        self,
        payload: ClassUpsertRequest,
        *,
        existing: ClassSummary | None = None,
    ) -> ClassUpsertRequest:
        normalized_meeting_url = self._normalize_meeting_url(payload.meeting_url)
        expects_zoom = is_zoom_delivery(payload.location_label, normalized_meeting_url)

        if expects_zoom and normalized_meeting_url and not is_zoom_meeting_url(normalized_meeting_url):
            raise ValueError("同步上課若使用 Zoom，meeting_url 必須是 zoom.us 的 Zoom 會議連結。")

        if expects_zoom and normalized_meeting_url is None and existing and existing.meeting_url:
            normalized_meeting_url = existing.meeting_url

        if expects_zoom and normalized_meeting_url is None:
            zoom_status = self.zoom_runtime.status()
            if zoom_status["ready"]:
                course = self.store.get_course(payload.course_slug)
                meeting = self.zoom_runtime.create_meeting(
                    topic=f"{course.name} - {payload.name}",
                    start_at=datetime.combine(payload.start_date, payload.start_time).astimezone(),
                    duration_minutes=self._duration_minutes(payload),
                    agenda=f"{course.name} / {payload.weekday} / {payload.name}",
                )
                normalized_meeting_url = str(meeting["join_url"])
            elif self._is_local_demo_mode():
                normalized_meeting_url = self._local_demo_zoom_url(payload)
            else:
                raise ValueError(
                    "這堂課設定為 Zoom 同步上課，但尚未提供 Zoom meeting_url，且 Zoom API 憑證尚未配置完成。"
                )

        normalized_location = (payload.location_label or "").strip()
        if normalized_meeting_url and is_zoom_meeting_url(normalized_meeting_url) and "zoom" not in normalized_location.lower():
            normalized_location = f"{normalized_location} / Zoom" if normalized_location else "Zoom Live"

        return payload.model_copy(
            update={
                "location_label": normalized_location or payload.location_label,
                "meeting_url": normalized_meeting_url,
            }
        )
    ...
    def create_class(self, payload: ClassUpsertRequest) -> ClassSummary:
        return self.store.create_class(self._resolve_class_payload(payload))

    def update_class(self, class_id: UUID, payload: ClassUpsertRequest) -> ClassSummary:
        return self.store.update_class(
            class_id,
            self._resolve_class_payload(payload, existing=self.store.get_class_by_id(class_id)),
        )
...
class AiAssistantService:
    ...
    @staticmethod
    def _infer_translation_languages(source_text: str, lesson_focus: str | None = None) -> tuple[str, str]:
        text = source_text.strip()
        has_japanese = any("\u3040" <= char <= "\u30ff" or "\u4e00" <= char <= "\u9fff" for char in text)
        has_ascii = any(char.isascii() and char.isalpha() for char in text)
        if has_japanese and not has_ascii:
            return "ja", "zh-Hant"
        if has_ascii and not has_japanese:
            return "zh-Hant", "ja"
        if lesson_focus and any(keyword in lesson_focus for keyword in ("日文", "日語", "Japanese")):
            return "zh-Hant", "ja"
        return "zh-Hant", "ja"

    def bidirectional_translation(self, payload: BiDirectionalTranslationRequest) -> BiDirectionalTranslationResponse:
        source_language, target_language = self._infer_translation_languages(payload.source_text, payload.lesson_focus)
        fallback = BiDirectionalTranslationResponse(
            source_text=payload.source_text.strip(),
            source_language=source_language,
            target_language=target_language,
            forward_translation=payload.source_text.strip(),
            reverse_translation=payload.source_text.strip(),
            teaching_notes=[
                "先讓老師確認這句話在課堂上的真實用途，再進行雙向翻譯。",
                "翻譯後要補上文化說法、口語語氣與適合學生程度的替代說法。",
                "若學生卡住，可以先回到關鍵字，再做情境口說。",
            ],
            classroom_prompts=[
                "請先用中文說一次，再用日文說一次。",
                "老師示範後，讓學生用自己的話重說一次。",
                "把原句拆成三個關鍵詞後，再進行雙向翻譯練習。",
            ],
            review_checks=[
                "確認句子是否符合學生程度",
                "確認日文是否自然且能在真實課堂使用",
        ...
        result, provider, reason = self.ai_runtime.enhance_model(
            feature_name="bidirectional_translation",
            instructions=(
                "Translate the source text in both directions for classroom use. "
                "Keep the result practical for teachers planning lessons, and include teaching notes that explain how to reuse it in class."
            ),
            context={
                ...
                "lesson_focus": payload.lesson_focus,
                "classroom_context": payload.classroom_context,
                "source_language": source_language,
                "target_language": target_language,
            },
            fallback_model=fallback,
        ...
        self.store.create_ai_log(
            module_name="teaching",
            action_name="bidirectional_translation",
            actor_email=payload.teacher_name,
            input_summary=f"source={source_language}->{target_language}, len={len(payload.source_text.strip())}",
            output_summary=f"provider={provider}, reason={reason}, notes={len(result.teaching_notes)}",
        )
        return result
...
class PlatformStatusService:
    ...
    def storage_summary(self) -> dict[str, object]:
        ...
        zoom_provider = SchoolPlatformZoomRuntime(self.store.settings).status()
        open_classes = self.store.open_classes()
        zoom_classes = [item for item in open_classes if is_zoom_delivery(item.location_label, item.meeting_url)]
        zoom_classes_with_links = [item for item in zoom_classes if item.meeting_url]
        zoom_classes_missing_links = [item for item in zoom_classes if not item.meeting_url]
        return {
            ...
            "live_class_delivery": {
                "provider": "zoom",
                "zoom_class_count": len(zoom_classes),
                "zoom_classes_with_links": len(zoom_classes_with_links),
                "zoom_classes_missing_links": len(zoom_classes_missing_links),
                "missing_link_classes": [f"{item.name} ({item.weekday})" for item in zoom_classes_missing_links[:8]],
                "message": (
                    "所有 Zoom 班級都已具備可進入的 meeting_url。"
                    if not zoom_classes_missing_links
                    else "仍有 Zoom 班級缺少 meeting_url，學員與老師無法直接進入同步教室。"
                ),
            },
        }
    ...
    def launch_readiness(self) -> dict[str, object]:
        ...
        add_check(
            "Zoom class links",
            "ready" if live_class_delivery["zoom_classes_missing_links"] == 0 else "blocker",
            str(live_class_delivery["message"]),
        )
        add_check(
            "Zoom auto-create integration",
            "ready" if bool(zoom_provider["ready"]) else "warning",
            str(zoom_provider["message"]),
        )
        add_check(
            "LINE notifications",
        ...
        return {
            ...
            "manual_tasks": [
                ...
                "若要自動建立 Zoom 會議，補齊 Zoom Server-to-Server OAuth 憑證。",
    ...
    def operational_readiness(self) -> dict[str, object]:
        ...
        add_check(
            "Zoom integration mode",
            "ready" if bool(zoom_provider["ready"]) or live_class_delivery["zoom_classes_missing_links"] == 0 else "warning",
            f"目前 Zoom 模式 = {zoom_provider['mode']}。",
        )
        add_check(
        ...
        if live_class_delivery["zoom_classes_missing_links"] > 0:
            launch_cutover_gaps.append("Zoom 班級 meeting_url")
        add_check(
        ...
        if live_class_delivery["zoom_classes_missing_links"] > 0:
            external_gaps.append("Zoom 班級正式連結")
        return {
