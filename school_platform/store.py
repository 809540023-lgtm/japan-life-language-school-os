...
def _demo_zoom_meeting_url(class_item: ClassSummary) -> str:
    digest = hashlib.sha1(f"{class_item.course_slug}:{class_item.name}:{class_item.weekday}".encode("utf-8")).hexdigest()
...
class SchoolPlatformStore:
    ...
    def _is_local_demo_mode(self) -> bool:
        base_url = str(self.settings.app_base_url)
        return "127.0.0.1" in base_url or "localhost" in base_url

    def _backfill_local_demo_zoom_links(self) -> bool:
        if not self._is_local_demo_mode():
            return False
        changed = False
        updated_classes: list[ClassSummary] = []
        for item in self.classes:
            if item.meeting_url or "zoom" not in item.location_label.lower():
                updated_classes.append(item)
                continue
            updated_classes.append(item.model_copy(update={"meeting_url": _demo_zoom_meeting_url(item)}))
            changed = True
        if changed:
    ...
    def _seed_users(self) -> list[UserAccount]:
        if not (self.settings.allow_default_seed_users or self._is_local_demo_mode()):
            return []
    ...
    def _load_or_seed(self) -> None:
        ...
        if not payload:
            self._backfill_local_demo_zoom_links()
            self._persist()
            return
        needs_persist = False
        ...
        if loaded_classes:
            ...
            needs_persist = self._backfill_local_demo_zoom_links() or needs_persist
        else:
            self._backfill_local_demo_zoom_links()
            needs_persist = True
    ...
    def hash_password(password: str) -> str:
        salt = hashlib.sha256(os.urandom(18)).hexdigest()[:18]
        ...
        return f"pbkdf2_sha256$180000${salt}${hashed}"
    ...
    def verify_password(self, password: str, password_hash: str) -> bool:
        ...
        if password_hash.startswith("pbkdf2_sha256$"):
            try:
                _, iterations_str, salt, stored_hash = password_hash.split("$", 3)
